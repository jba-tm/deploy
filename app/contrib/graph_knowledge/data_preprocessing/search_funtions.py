...
def search_drug_gk(query: str, drugs_df: pd.DataFrame, kg_df: pd.DataFrame):
...
def search_clinical_data(query: str, final_cl_kg_searched_final: pd.DataFrame, expanded_df_final: pd.DataFrame,
                         edge_filters: List[str] = None):
    query = query.strip()
    ...
    if query.startswith('NCT'):
        ...
        matched_nct_numbers = final_cl_kg_searched_final[
            final_cl_kg_searched_final['target'].isin(similar_drugs)
        ]['source'].unique()
        ...
        matched_indices_2 = final_cl_kg_searched_final[
            final_cl_kg_searched_final['source'].isin(matched_nct_numbers)
        ].index
